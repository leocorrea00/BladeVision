...
let GAS_SPECIFIC_GRAVITY = 0.554; // Relative to air (Methane ≈ 0.554)
...
let CHOKE_PRESSURE_psi = 0; // User-set pressure (acts as backpressure when UP, or SPP when DOWN)
...
let KILL_PRESSURE_psi = 0; // User-set pressure (acts as backpressure when UP, or SPP when DOWN)
...
function updateFormationFluidMigration(dt_s) {
  ...
  if (!formationFluidEnabled) return;
  ...
  current_influx_m3s = influx_m3s;

  if (influxRate_gpm <= 0) {
    // Even with no influx, migrate existing kick fluids
  ...
  if (formationFluidDepth_m >= BOP_DEPTH) {
    ...
    if (cellIndex >= 0 && cellIndex < N_BELOW) {
      const cellVolume = areaAtDepthTop_(formationFluidDepth_m) * CELL_M;
      const fraction = influx_volume_m3 / Math.max(cellVolume, 1e-9);

      // Add to appropriate phase array based on fluid type
      if (formationFluidType === 'GAS') {
        gasPhaseFrom_below[cellIndex] = Math.min((gasPhaseFrom_below[cellIndex] || 0) + fraction, 1.0);
      } else if (formationFluidType === 'OIL' || formationFluidType === 'BLACKOIL') {
        oilPhaseFrom_below[cellIndex] = Math.min((oilPhaseFrom_below[cellIndex] || 0) + fraction, 1.0);
      } else if (formationFluidType === 'WATER') {
        waterPhaseFrom_below[cellIndex] = Math.min((waterPhaseFrom_below[cellIndex] || 0) + fraction, 1.0);
      }
    }
  } else {
    // Above BOP (riser)
    const cellIndex = Math.floor(formationFluidDepth_m / CELL_M);
    if (cellIndex >= 0 && cellIndex < N_ABV) {
      const cellVolume = areaAtDepthTop_(formationFluidDepth_m) * CELL_M;
      const fraction = influx_volume_m3 / Math.max(cellVolume, 1e-9);

      // Add to appropriate phase array based on fluid type
      if (formationFluidType === 'GAS') {
        gasPhaseFrom_above[cellIndex] = Math.min((gasPhaseFrom_above[cellIndex] || 0) + fraction, 1.0);
      } else if (formationFluidType === 'OIL' || formationFluidType === 'BLACKOIL') {
        oilPhaseFrom_above[cellIndex] = Math.min((oilPhaseFrom_above[cellIndex] || 0) + fraction, 1.0);
      } else if (formationFluidType === 'WATER') {
        waterPhaseFrom_above[cellIndex] = Math.min((waterPhaseFrom_above[cellIndex] || 0) + fraction, 1.0);
      }
    }
  }
  ...
  // Update front position (find highest cell with any formation fluid)
  let highestCell = -1;
  for (let i = 0; i < N_BELOW; i++) {
...
function stepAdvection(){
  ...
  if (qDown_m3s > FLOW_THRESHOLD) {
    ...
    while (accBelow >= CELL_M){
      ...
      // Advect formation fluids DOWN in parallel with mud
      for (let i = gasPhaseFrom_below.length - 1; i > 0; i--) {
      ...
      // No formation fluid enters from booster (clean mud)
  ...
  if (qUp_m3s > FLOW_THRESHOLD) {
    ...
    while (accDSup >= CELL_M && cellsFlowing > 0){
      ...
      // Advect formation fluids UP in parallel with mud
      for (let i = 0; i < gasPhaseFrom_below.length - 1; i++) {
        gasPhaseFrom_below[i] = gasPhaseFrom_below[i + 1];
        oilPhaseFrom_below[i] = oilPhaseFrom_below[i + 1];
        waterPhaseFrom_below[i] = waterPhaseFrom_below[i + 1];
      }
      // No formation fluid enters from drillstring at bottom (clean mud)
  ...
  if (qUp_m3s > FLOW_THRESHOLD) {
    ...
    while (accAbove >= CELL_M){
      ...
      const fromBelowEff = effectiveCellDensity_ppg(fromBelowMW, 0, false);
      const boosterEff = boosterEffluentMW;
      const qTotal = Math.max(qFromAnnBelow + qBO_up, 1e-12);
      const mixMW = (qFromAnnBelow * fromBelowEff + qBO_up * boosterEff) / qTotal;
      ...
      // Booster is clean mud, so dilute the formation fluids by flow rate ratio
      const dilutionFactor = clamp(qFromAnnBelow / Math.max(qTotal, 1e-12), 0, 1);
  ...
  // ===============================
  if (BOP_CLOSED && Q_CHOKE > 0 && CHOKE_LINE_OPEN) {
    ...
    if (CHOKE_DIRECTION === "DOWN") {
      // Fluid flows DOWN: Surface → BOP → Below-BOP annulus
      ...
      // Fluid flows UP: Below-BOP annulus → BOP → Surface
      ...
      while (accChoke >= CELL_M) {
        ...
        // Fluid entering from below-BOP annulus at BOP depth
        // Take fluid from cell at BOP depth in annBelowGrid (top cell = index 0)
        const sourceMW = annBelowGrid[0] || MW_DS;
  ...
  // ===============================
  if (BOP_CLOSED && Q_KILL > 0 && KILL_LINE_OPEN) {
...
function applyGasExpansion(dt_s) {
  ...
  for (let i = 0; i < gasPhaseFrom_below.length; i++) {
    ...
    const cellDepth = BOP_DEPTH + (gasPhaseFrom_below.length - 1 - i) * CELL_HEIGHT;
...
function effectiveCellDensity_ppg(mudWeight_ppg, cellIndex, isAboveBOP) {
  ...
  if (totalKickFraction > 0.001) {
    ...
    // Mix with mud: (mud fraction × mud density) + (kick fraction × kick density)
...
function ecdAtDepth(d){
  ...
  // ✅ NOW INCLUDES: Kick fluid density mixing (lighter fluids reduce hydrostatic)
...
function pressureAtDepth(d){
  ...
  // SBP (only when riser is FULL and BOP OPEN)
  const riserFull = (fluidTop <= 0);
  const P_sbp = (MPD_ON && riserFull && !BOP_CLOSED) ? (CURRENT_SBP || 0) : 0;

  // BOP Line Contribution (when BOP closed and choke/kill lines flowing UP)
  const P_bop_lines = bopLinePressureContribution();

  // Closed Well Pressure Boost (when pumping to closed well with no exit)
  const P_closed_well = BOP_CLOSED ? (closedWellPressureBoost_psi || 0) : 0;

  const total = P_hydro + P_fric + P_sbp + P_bop_lines + P_closed_well;
  return total;
}
...
function chokeFrictionPsiLength(len_m, flowRate_m3s){
  ...
  // ✅ FIXED: Use pipe friction function (not annulus with OD=0)
  ...
  const PA_TO_PSI = 1.0 / 6894.757;
  return dpdm_Pa * len_m * PA_TO_PSI;
}
...
// When lines flow DOWN, this acts as pump pressure (SPP equivalent)
function bopLinePressureContribution(){
  ...
  // Choke Line
  if (CHOKE_DIRECTION === "UP" && Q_CHOKE > 0 && CHOKE_LINE_OPEN) {
    const qChoke_m3s = Q_CHOKE / 15850.323;
    const friction_psi = chokeFrictionPsiLength(BOP_DEPTH, qChoke_m3s);
    // When flowing UP: Friction + Applied Choke Pressure adds to BHP
    totalContribution += friction_psi + CHOKE_PRESSURE_psi;
  }

  // Kill Line
  if (KILL_DIRECTION === "UP" && Q_KILL > 0 && KILL_LINE_OPEN) {
    const qKill_m3s = Q_KILL / 15850.323;
    const friction_psi = killFrictionPsiLength(BOP_DEPTH, qKill_m3s);
    // When flowing UP: Friction + Applied Kill Pressure adds to BHP
    totalContribution += friction_psi + KILL_PRESSURE_psi;
  }
...
function calculateUPFlowDistribution(qAvailable_m3s, BHP_source_psi) {
  ...
  if (!BOP_CLOSED) {
    ...
    const appliedPressure = (MPD_ON && (annulusTopDepthGlobal() <= 0.5)) ? (CURRENT_SBP || 0) : 0;
  ...
  for (let p of paths) {
    // Simplified conductance estimate based on Hagen-Poiseuille for initial guess
    const D = p.diameter_m;
    const L = p.length_m;
    // Conductance ~ D^4 / (viscosity * L)
    // Approximate viscosity from K for power law
    const approxVisc = K * Math.pow(1.0, n-1); // at reference shear rate
    p.conductance = Math.pow(D, 4) / (approxVisc * L + 1e-9);
  }
  ...
  for (let iter = 0; iter < MAX_ITERATIONS; iter++) {
    ...
    const sourcePressures = paths.map((p, i) => {
      ...
      let frictionPsi = 0;
      if (p.name === 'RISER') {
        // Use annulus friction calculation
        const A_riser = area_annulus(riser_ID_m, ds_OD_m);
...
// ✅ NOW HANDLES BOP CLOSED STATE: Riser isolated from below-BOP annulus
...
function routeAnnulusFlowsWithLosses(dt_s){
  ...
  const qInflux = (typeof current_influx_m3s === 'number') ? current_influx_m3s : 0;
  ...
  if (BOP_CLOSED) {
    ...
    let qIn_belowBOP = qDS + qInflux; // DS return + kicks enter below BOP
    ...
    // Losses consume fluid from below-BOP annulus
    const qToLoss = Math.min(qDem, Math.max(0, qIn_belowBOP));
    ...
    if (CHOKE_LINE_OPEN && CHOKE_DIRECTION === 'UP' && qChoke_actual > 1e-6) {
      // Calculate actual pressure at surface with calculated flow
      const actualFriction = chokeFrictionPsiLength(BOP_DEPTH, qChoke_actual);
      const chokeMW = chokeGrid.length > 0 ? average(chokeGrid) : MW_CHOKE;
      const actualHydro = PSI_COEF * chokeMW * BOP_DEPTH;
      const actualSurfaceP = BHP_at_BOP - actualHydro - actualFriction;
      const pressureDiff = Math.abs(actualSurfaceP - CHOKE_PRESSURE_psi);
      if (pressureDiff > 50) {
        window.__CHOKE_PRESSURE_WARNING = `Target: ${CHOKE_PRESSURE_psi.toFixed(0)} psi, Actual: ${actualSurfaceP.toFixed(0)} psi`;
      }
    }

    if (KILL_LINE_OPEN && KILL_DIRECTION === 'UP' && qKill_actual > 1e-6) {
      // Calculate actual pressure at surface with calculated flow
      const actualFriction = killFrictionPsiLength(BOP_DEPTH, qKill_actual);
      const killMW = killGrid.length > 0 ? average(killGrid) : MW_KILL;
      const actualHydro = PSI_COEF * killMW * BOP_DEPTH;
      const actualSurfaceP = BHP_at_BOP - actualHydro - actualFriction;
      const pressureDiff = Math.abs(actualSurfaceP - KILL_PRESSURE_psi);
      if (pressureDiff > 50) {
        window.__KILL_PRESSURE_WARNING = `Target: ${KILL_PRESSURE_psi.toFixed(0)} psi, Actual: ${actualSurfaceP.toFixed(0)} psi`;
      }
    }
    ...
    const netVolumeTrapped_m3s = Math.max(0, qIn_belowBOP - qToLoss - qToSurface_belowBOP);
    const wellIsClosed = (qToSurface_belowBOP < 0.001) && (netVolumeTrapped_m3s > 0.001);

    if (wellIsClosed) {
      // Well is closed - fluid has nowhere to go, pressure increases
      // Convert trapped volume to pressure increase
      // Using simplified compressibility: ~10 psi per bbl in a typical wellbore
      const dt_s = Math.max(1/120, window.dt_s || 1/60); // time step in seconds
      const trappedVolume_bbl = netVolumeTrapped_m3s * 6.28981 * dt_s; // m3/s to bbl
      const pressureIncrease_psi = trappedVolume_bbl * 15; // ~15 psi per bbl trapped
      closedWellPressureBoost_psi += pressureIncrease_psi;
    } else {
      // Well has exit path - pressure boost decays back to normal
      closedWellPressureBoost_psi *= 0.95; // Decay factor
    }
    ...
    window.__qAnnDown_m3s = 0; // No downward flow when BOP closed

    // Flow Out calculation - from choke/kill lines
    const targetFO_gpm = Math.max(0, qToSurface_belowBOP * 15850.323);

    // Update FLOW_OUT with fast filter (or immediate if well is closed)
    if (wellIsClosed) {
      // Well is closed - set FO to 0 immediately (no filter delay)
      FLOW_OUT_gpm = 0;
    } else {
      // Normal filter for smooth FO changes
      const ALPHA = 0.5;
      const DQDT_MAX = 1000;
  ...
  // ✅ KICK INFLUX: Add formation fluid influx to upward flow
  ...
  let qAnnUp = DS_left + TF_left + (riserFull || takeBO === 0 ? BO_left : 0) + qInflux;
  ...
  if (hasChokeUP || hasKillUP) {
    ...
    if (hasChokeUP) {
      ...
      const chokeMW = chokeGrid.length > 0 ? average(chokeGrid) : MW_CHOKE;
      const friction = chokeFrictionPsiLength(BOP_DEPTH, Q_choke_actual);
      const hydro = PSI_COEF * chokeMW * BOP_DEPTH;
      const actualSurface = BHP_at_BOP - hydro - friction;
      if (Math.abs(actualSurface - CHOKE_PRESSURE_psi) > 50) {
        window.__CHOKE_PRESSURE_WARNING = `Target: ${CHOKE_PRESSURE_psi.toFixed(0)} psi, Actual: ${actualSurface.toFixed(0)} psi`;
      }
    }
    ...
    if (hasKillUP) {
      ...
      const killMW = killGrid.length > 0 ? average(killGrid) : MW_KILL;
      const friction = killFrictionPsiLength(BOP_DEPTH, Q_kill_actual);
      const hydro = PSI_COEF * killMW * BOP_DEPTH;
      const actualSurface = BHP_at_BOP - hydro - friction;
      if (Math.abs(actualSurface - KILL_PRESSURE_psi) > 50) {
        window.__KILL_PRESSURE_WARNING = `Target: ${KILL_PRESSURE_psi.toFixed(0)} psi, Actual: ${actualSurface.toFixed(0)} psi`;
      }
    }
  } else {
...
function buildGelAndGeometryUI(x, y){
  ...
  // === BOP WELL CONTROL LINES (Booster, Choke, Kill) ===
  y2 += 40;
  mkLabel("Booster ID (in)", x+360, y2-16);
  const boosterIdIn = createInput(String(booster_ID_in),'number').position(x+360, y2).size(70);
  boosterIdIn.input(() => { booster_ID_in = num(boosterIdIn.value()); booster_ID_m = booster_ID_in * 0.0254; });

  mkLabel("Choke ID (in)", x+500, y2-16);
  const chokeIdIn = createInput(String(CHOKE_LINE_ID_in),'number').position(x+500, y2).size(70);
  chokeIdIn.input(() => { CHOKE_LINE_ID_in = num(chokeIdIn.value()); CHOKE_LINE_ID_m = CHOKE_LINE_ID_in * 0.0254; });

  y2 += 40;
  mkLabel("Kill ID (in)", x+360, y2-16);
  const killIdIn = createInput(String(KILL_LINE_ID_in),'number').position(x+360, y2).size(70);
...
// Always visible, positioned next to booster line
{
  ...
  // Color based on direction and flow state
  const killIsFlowing = Q_KILL > 0 && KILL_LINE_OPEN;
  let killHue, killSat, killBright, killAlpha;
  if (killIsFlowing) {
    if (KILL_DIRECTION === "UP") {
      killHue = 200; killSat = 80; killBright = 90; killAlpha = 200; // Blue
    } else {
      killHue = 30; killSat = 90; killBright = 90; killAlpha = 200; // Orange
    }
  } else {
    // No flow or closed - gray dormant
    killHue = 0; killSat = 0; killBright = 60; killAlpha = 120;
  }

  // Draw line pipe
  stroke(killHue, killSat, killBright - 20, killAlpha);
  strokeWeight(2);
  noFill();
  rect(killLineX - killLineW/2, yTop, killLineW, yBot - yTop, 3);

  // Fill with fluid color if flowing
  if (killIsFlowing) {
    noStroke();
    fill(killHue, killSat, killBright, killAlpha - 20);
    rect(killLineX - killLineW/2 + 1, yTop, killLineW - 2, yBot - yTop);

    // Draw flow arrows
    noFill();
    stroke(killHue, killSat, 100, 255);
    strokeWeight(2);
    const numArrows = 3;
    const arrowSpacing = (yBot - yTop) / (numArrows + 1);
    for (let i = 1; i <= numArrows; i++) {
      const arrowY = KILL_DIRECTION === "UP"
        ? yBot - i * arrowSpacing
        : yTop + i * arrowSpacing;
      const arrowDir = KILL_DIRECTION === "UP" ? -1 : 1;
      const arrowSize = 6;
  ...
  rotate(-HALF_PI);
  noStroke();
  fill(killHue, killSat, killBright);
  textSize(10);
  textAlign(CENTER, CENTER);
  if (killIsFlowing) {
...
// Always visible, positioned on right side of well
{
  ...
  let chokeHue, chokeSat, chokeBright, chokeAlpha;
  if (chokeIsFlowing) {
    if (CHOKE_DIRECTION === "UP") {
      chokeHue = 200; chokeSat = 80; chokeBright = 90; chokeAlpha = 200; // Blue
    } else {
      chokeHue = 30; chokeSat = 90; chokeBright = 90; chokeAlpha = 200; // Orange
    }
  } else {
    // No flow or closed - gray dormant
    chokeHue = 0; chokeSat = 0; chokeBright = 60; chokeAlpha = 120;
  }

  // Draw line pipe
  stroke(chokeHue, chokeSat, chokeBright - 20, chokeAlpha);
  strokeWeight(2);
  noFill();
  ...
  if (chokeIsFlowing) {
    noStroke();
    fill(chokeHue, chokeSat, chokeBright, chokeAlpha - 20);
    rect(chokeLineX - chokeLineW/2 + 1, yTop, chokeLineW - 2, yBot - yTop);

    // Draw flow arrows
    noFill();
    stroke(chokeHue, chokeSat, 100, 255);
    strokeWeight(2);
    const numArrows = 3;
    const arrowSpacing = (yBot - yTop) / (numArrows + 1);
    for (let i = 1; i <= numArrows; i++) {
      const arrowY = CHOKE_DIRECTION === "UP"
        ? yBot - i * arrowSpacing
        : yTop + i * arrowSpacing;
      const arrowDir = CHOKE_DIRECTION === "UP" ? -1 : 1;
      const arrowSize = 6;
  ...
  rotate(-HALF_PI);
  noStroke();
  fill(chokeHue, chokeSat, chokeBright);
  textSize(10);
  textAlign(CENTER, CENTER);
  if (chokeIsFlowing) {
...
function drawReferenceBadges(){
  ...
  for (const m of MARKS){
    ...
    drawBadgeSm(well.x + well.w/2, y, `${m.name} • ${nf(p,0,0)} psi`, `Eff. ECD • ${nf(ecdEff,1,2)} ppg`);
  }
}
...
function drawGaugesAndCharts(){
  ...
  if (levelDeficit_m > 0.01){
    ...
    text(`DS level ↓ ${nf(levelDeficit_m,1,1)} m`, cxSPP - 48, cyTop + 40);
    textAlign(CENTER, CENTER);
  }
  ...
  if (levelDeficit_bo_m > 0.01){
    textAlign(LEFT, CENTER); textSize(11); fill(0,0,95);
    text(`Bo level ↓ ${nf(levelDeficit_bo_m,1,1)} m`, cxBo - 32 , cyTop + 40);
    textAlign(CENTER, CENTER);
  }
...
// --- Kick markers on PRESSURE chart (triangle at Pore Pressure×Depth) ---
{
...
// --- Kick markers on ECD chart (triangle at formation fluid density×Depth) ---
{
  if (formationFluidEnabled && formationFluidDepth_m > 0) {
    const xL = ex + padL, xR = ex + wE - padR;
    const yT = ey + padT, yB = ey + h - padB;
    ...
    // Show formation fluid density in ppg
    const fluidProps = FORMATION_FLUID_TYPES[formationFluidType];
    const fluid_ppg = fluidProps ? fluidProps.density_ppg : 8.33;

    const x = map(fluid_ppg, eMin, eMax, xL, xR, true);
    ...
    textSize(10); textAlign(LEFT, BOTTOM);
    const typeStr = fluidProps ? fluidProps.name : 'Kick';
    text(`${typeStr}: ${nf(fluid_ppg,1,2)} ppg @ ${nf(formationFluidDepth_m,1,0)} m`, x + 10, y - 2);
    pop();
  }
}
...
function buildEnhancedHydraulicsUI(parentDiv){
  const card = createDiv('').parent(parentDiv)
    .style('margin-top', '16px')
    .style('padding', '10px')
    .style('border', '1px solid rgba(140,200,255,0.25)')
    .style('border-radius', '8px')
    .style('background', 'rgba(12,26,42,0.6)');

  createDiv('Enhanced Hydraulics').parent(card)
    .style('font-weight', 'bold')
    .style('color', '#8cd7ff')
    .style('margin-bottom', '8px');

  const mkToggleRow = (key, label) => {
    const row = createDiv('').parent(card)
      .style('display', 'flex')
  ...
  const pOnlyRow = createDiv('').parent(card)
    .style('display', 'grid')
  ...
  const gravRow = createDiv('').parent(card)
    .style('display', 'flex')
    .style('align-items', 'center')
    .style('gap', '10px');
...
function buildLossesUI() {
  ...
  const panelWidth = bottomPanelCardWidth();

  uiLosses.root = styleBottomPanelCard(createDiv('').position(pos.x, pos.y), '#2f4a5f')
...
function styleBottomPanelCard(div, accent = null) {
  div
    .style('padding', '12px')
    ...
    .style('color', '#EEE')
    .style('width', `${bottomPanelCardWidth()}px`);
...
function buildBOPWellControlUI() {
  ...
  uiBOP.root = styleBottomPanelCard(createDiv('').position(pos.x, pos.y), '#1f3b57')
    .style('width', `${panelWidth}px`);
  ...
  const chokePress = makeSliderRow(chokeCard, 'Pressure (psi)', 0, 5000, CHOKE_PRESSURE_psi, 50, 0);
  uiBOP.chokePressSlider = chokePress.slider;
  uiBOP.chokePressInput = chokePress.input;
  chokePress.slider.input(() => {
    CHOKE_PRESSURE_psi = uiBOP.chokePressSlider.value();
    uiBOP.chokePressInput.value(nf(CHOKE_PRESSURE_psi, 1, 0));
  });
  chokePress.input.input(() => {
    CHOKE_PRESSURE_psi = Math.max(0, Number(uiBOP.chokePressInput.value()) || 0);
  ...
  const killPress = makeSliderRow(killCard, 'Pressure (psi)', 0, 5000, KILL_PRESSURE_psi, 50, 0);
  uiBOP.killPressSlider = killPress.slider;
  uiBOP.killPressInput = killPress.input;
  killPress.slider.input(() => {
    KILL_PRESSURE_psi = uiBOP.killPressSlider.value();
    uiBOP.killPressInput.value(nf(KILL_PRESSURE_psi, 1, 0));
  });
  killPress.input.input(() => {
    KILL_PRESSURE_psi = Math.max(0, Number(uiBOP.killPressInput.value()) || 0);
  ...
  const hydCard = createDiv('').parent(uiBOP.root)
    .style('margin-top', '16px')
    .style('padding', '10px')
    .style('border-radius', '8px')
    .style('background', 'rgba(10,25,35,0.6)');

  createDiv('Enhanced Hydraulics Controls').parent(hydCard)
    .style('font-weight', 'bold')
    .style('color', '#7fd5ff')
    .style('margin-bottom', '6px');

  buildEnhancedHydraulicsUI(hydCard);
...
function updateChokePressureInputState() {
  if (!uiBOP.chokePressInput || !uiBOP.chokePressSlider) return;

  if (CHOKE_DIRECTION === 'DOWN') {
    // DOWN = Pumping mode → SPP is calculated (read-only)
    ...
    // UP = Extraction mode → User sets backpressure (editable)
...
// Update kill pressure input state based on direction
function updateKillPressureInputState() {
...
function updateBOPPressureDisplays() {
  ...
  // CHOKE LINE
  if (CHOKE_DIRECTION === 'DOWN' && Q_CHOKE > 0 && BOP_CLOSED && CHOKE_LINE_OPEN) {
    // Calculate SPP for choke line
    ...
    // UP mode: user controls pressure, just ensure it's not overwritten
    // (pressure is set by user input handler)
  }

  // KILL LINE
  if (KILL_DIRECTION === 'DOWN' && Q_KILL > 0 && BOP_CLOSED && KILL_LINE_OPEN) {
    // Calculate SPP for kill line
    ...
    // UP mode: user controls pressure
  }
}
...
function updateBOPFlowDisplays() {
  ...
  if (CHOKE_DIRECTION === 'UP') {
    ...
    const calcFlow_m3s = window.__Q_CHOKE_CALCULATED_m3s || 0;
    const calcFlow_gpm = calcFlow_m3s * 15850.323;
  ...
  if (KILL_DIRECTION === 'UP') {
    // UP: Show calculated flow (pressure-driven)
    const calcFlow_m3s = window.__Q_KILL_CALCULATED_m3s || 0;
    const calcFlow_gpm = calcFlow_m3s * 15850.323;
...
function buildKicksUI() {
  ...
  uiKicks.root = styleBottomPanelCard(createDiv('').position(pos.x, pos.y), '#5b2f2f')
    .style('width', `${panelWidth}px`)
  ...
  createDiv(
    `<div style="margin-top:8px;font-size:9px;opacity:.65;line-height:1.2">
      ...
      <b>Pressure:</b> Q = PI × (P<sub>pore</sub> - BHP).<br/>
